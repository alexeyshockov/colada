 ...
 * @mixin HashSet
 */
class HashSetSpec extends ObjectBehavior
{
    ...
    function it_can_be_created_through_the_function()
    {
        $this->beConstructedThrough(function () {
            return \Colada\hash_set(1, 2, 3);
        });

        $this->shouldHaveType(HashSet::class);
        $this->shouldHaveValues([1, 2, 3]);
    }
    ...
    function it_can_be_created_from_result()
    {
        ...
        $this->shouldHaveValues($source);
    }

    function it_do_not_contain_duplicates()
    {
        $this->beConstructedWith([1, 2, 3, 3, 2, 1]);

        $this->shouldHaveValues([1, 2, 3]);
    }

    function it_should_have_generic_hasher_by_default()
    {
        $this->getHasher()->shouldBe(\Colada\any_hash);
    }
    ...
    function it_can_be_mapped()
    {
        ...
        $result->shouldHaveType(HashSet::class);
        $result->shouldHaveValues([2, 4, 6, 8, 10, 12]);
    }
    ...
    function it_can_be_flatten()
    {
        ...
        $result->shouldHaveValues([1, 2, 3, 4, 5, 6]);
    }
    ...
    function it_can_be_filtered()
    {
        ...
        $selected->shouldHaveType(HashSet::class);
        $selected->shouldContains([2, 4, 6]);

        $rejected = $this->reject($isEven);
        $rejected->shouldHaveType(HashSet::class);
        $rejected->shouldContains([1, 3, 5]);
    }

    function it_can_drop_first_values()
    {
        $result = $this->drop(3);

        $result->shouldHaveType(HashSet::class);
        $result->shouldContains([4, 5, 6]);
    }

    function it_can_take_only_first_values()
    {
        $result = $this->take(3);
    ...
    {
        $this->reversed()->shouldContains([6, 5, 4, 3, 2, 1]);
    }
