...
class SequenceSpec extends ObjectBehavior
{
    ...
    function it_can_be_created_through_the_function()
    {
        $this->beConstructedThrough(function () {
            return \Colada\seq(1, 2, 3);
        });

        $this->shouldHaveType(Sequence::class);
        $this->shouldContains([1, 2, 3]);
    }

    function it_can_wrap_spl_fixed_array()
    {
        $source = [1, 2, 3];
        $fixedArray = SplFixedArray::fromArray($source);

        $this->beConstructedThrough('wrap', [$fixedArray]);
    ...
    function it_can_be_mapped()
    {
        ...
        $result->shouldHaveType(Sequence::class);
        $result->shouldContains([2, 4, 6]);
    }
    ...
    function it_can_be_filtered()
    {
        ...
        $selected->shouldHaveType(Sequence::class);
        $selected->shouldContains([2, 4, 6]);

        $rejected = $this->reject($isEven);
        $rejected->shouldHaveType(Sequence::class);
